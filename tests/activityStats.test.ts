...
describe("classement", () => {
  ...
  it("reconnaît l'app elle-même, dont l'ancien processus s'appelait « tao »", () => {
    const res = classify("tao", "tao", []);
    /* « admin » et non plus « trading » : l'app a quitté la section trading du
       catalogue avec les pages qui la justifiaient. */
    expect(res.category).toBe("admin");
    expect(res.label).toBe("pingnouille");
  });
